...
from Assets.models import Assets, AssetsUser, IDC, SERV_STATUS, SERV_SORT
import subprocess
...
def addStatus(request):
    ...
    if request.method == 'POST':
        ...
        if status_name:
            status = SERV_STATUS(name=status_name)
            status.save()
            msg = u'%s 服务器状态添加成功' % status_name
        else:
            error = u'不能为空'
    return render_to_response('addStatus.html',
                              {'error': error, 'msg': msg, 'user_menu': 'active'},
                              context_instance=RequestContext(request))
...
def addSort(request):
    ...
    msg = ''
    if request.method == 'POST':
        sort_name = request.POST.get('name')
        if sort_name:
            sort = SERV_SORT(name=sort_name)
            sort.save()
            msg = u'%s 服务分类添加成功' % sort_name
        else:
            error = u'不能为空'
    return render_to_response('addSort.html',
                              {'error': error, 'msg': msg, 'user_menu': 'active'},
                              context_instance=RequestContext(request))
...
def addAssets(request):
    ...
    msg = ''
    idcs = IDC.objects.all()
    serv_statuses = SERV_STATUS.objects.all()
    serv_sorts = SERV_SORT.objects.all()
    if request.method == 'POST':
    ...
    return render_to_response('addAssets.html', {'msg': msg, 'error': error, 'idcs': idcs, 'serv_statuses': serv_statuses, 'serv_sorts': serv_sorts, 'asset_menu': 'active'},
                              context_instance=RequestContext(request))
